...
namespace uPLibrary.Networking.M2Mqtt.Messages
{
    ...
    public abstract class MqttMsgBase
    {
        ...
        /// <param name="channel">Channel from reading bytes</param>
        /// <returns>Decoded remaining length</returns>
        protected static int decodeRemainingLength(IMqttNetworkChannel channel, out byte[] rcvbuf)
        {
            int multiplier = 1;
            int value = 0;
            int digit = 0;
            ...
            rcvbuf = new byte[buf.Count];
            Array.Copy(buf.ToArray(), 0, rcvbuf, 0, buf.Count);
